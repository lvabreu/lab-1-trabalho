...
class MaxHeapBinaria:
    ...
    def __heapify_down(self, i):
        ...
        while True:
            ...
            if esq < n and self.__nos[esq].getDado() > self.__nos[maior].getDado():
                maior = esq

            if dir < n and self.__nos[dir].getDado() > self.__nos[maior].getDado():
                maior = dir

            if maior != i:
                temp = self.__nos[i].getDado()
                self.__nos[i].setDado(self.__nos[maior].getDado())
                self.__nos[maior].setDado(temp)
                i = maior
            else:
                break
